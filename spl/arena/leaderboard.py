...
def fallback_motto(sim: object) -> dict[str, str]:
    ...
    if sim.completed:
        return {"motto": "足るを知る者、冬を越ゆ。", "words": "島は何も約束せなんだが、裏切りもせなんだ。"}
    if "Dehydration" in tail or hero.water <= 0:
        return {"motto": "水を恃む前に、水を掘れ。", "words": "渇きは、いつも計画より一日早い。"}
    if "Starvation" in tail or hero.hunger <= 0:
        return {"motto": "飯は思想に先んず。", "words": "明日の備えを、今日の腹が追い越した。"}
    if "Winter" in tail or "snow" in tail.lower():
        return {"motto": "冬は約束どおりに来る。壁は約束より早く。", "words": "白い静寂に、火がひとつ足りなかった。"}
    return {"motto": "島は非情、されど公平なり。", "words": "敗れたのではない、学びが一年に間に合わなんだだけ。"}
