...
git clone https://github.com/Tonoken3/e-Choumei && cd e-Choumei
python3 -m spl pixel --difficulty 楽園   # マウスだけで遊べます(LLM不要)
```

- LLMで仙人を観戦したい人は「[LM Studioで遊ぶ](#lm-studioで遊ぶ)」へ。
- 記録に挑む人は `--difficulty 修羅`（既定）が正典——全記録の故郷です。
...
島は**ふたつ**だけ。`修羅`と`楽園`です。`修羅`では、どの隠者も**おおむね19日のうちに尽きます**——これは「同じ脳でどこまで延命できるか」を測るための**カノニカルな基準島**であり、**全記録の故郷**だからです（**今がもう修羅だから**——島は最初から修羅でした）。初めて触る人や、まずマウスだけで雰囲気を味わいたい人のために、夜ごとの消耗をやわらげた `楽園` を用意しました。

`--difficulty {楽園,修羅}` で選べます（`play` / `simulate` / `pixel` / `evolve`。既定は `修羅`）。

| きびしさ | 空腹/夜 | 水/夜 | 餓死HP | 渇死HP | 冬の追加税(家未改修) | 開始の追加物資 |
|---|---|---|---|---|---|---|
| **楽園**（はじめての方へ） | -10 | -14 | -7 | -10 | -2HP / -1正気 | 木の実+3・薪+2 |
| **修羅**（正典・記録はこちら） | -15 | -20 | -10 | -15 | -4HP / -2正気 | なし |

- **`修羅` は記録の正典**であり、**全記録の故郷**です。生存日数の記録やリーダーボード、冒険の書の比較はすべて `修羅` を基準にします。`修羅` の数値は難易度機能を入れる前の定数と**バイト単位で一致**しており、同じ seed なら以前とまったく同じ一年が再現されます。
- **入植のしおり（システムプロンプト）は嘘をつきません**。仙人に渡る「夜ごとの致死算術」は、実際に遊んでいる島の数値で動的に綴られます——`修羅` の隠者は `修羅` の算術を、`楽園` の隠者は緩い算術を読みます。
- 決定論は保たれます: きびしさは固定の消耗を倍率で増減させるだけで**乱数を引かない**ので、`(seed, difficulty)` が同じなら必ず同じ展開になります。
- リザルト画面（ターミナル版・ボクセル版とも）では、`楽園` のときだけ `きびしさ: 楽園` と表示され（`修羅` は既定なので無表示）、`--book` の冒険の書にも `difficulty` が記録されます。
- 旧名（`やさしい`→`楽園`、`ふつう`→`修羅`）も受け付けるので、過去のコマンドや記録はそのまま動きます。

```bash
python3 -m spl pixel    --difficulty 楽園               # 初めての人向け（LLM不要）
python3 -m spl simulate --difficulty 修羅 --seed 42      # 記録に挑む（正典・全記録の故郷）
```
...
観戦（見守る）と作戦（言葉で導く）の上に、もう一つの階層があります——**奇跡**。十分な**神力（しんりき）**を持つ人間プレイヤーは、世界そのものか、仙人の心に働きかける「奇跡」を**キュー（予約）**できます。腕（プレイヤーの裁量）で1年（112日）の踏破を可能にするための、**共同（co-op）プレイ**の仕組みです。

二つの掟がこの設計を貫きます:

1. **奇跡は世界か心に触れる。手には触れない。** 奇跡が仙人のために `GameAction` を出すことは決してありません——変えるのは天気・在庫・商人の到来、あるいは次の観測に囁く勅命だけ。手は最後まで仙人（か脳）自身のもの。だから手動モードが壊れません。
2. **神は夜に働く。** 奇跡はいつでもキューできますが、**今すぐは何も起きません**。効果はすべて**夜の境界**（その日の終わり）で解決します。この一つの規則が、ピクセルUIの保留中アクションとの競合・承認停止との混乱・リプレイの非決定性を一掃します。今夜の死は防げません（蘇生不可——試されるのは先見です）。

**神力経済（A-hybrid）**: 開始 **3**、**7日ごとの朝に +1**、保有上限 **5**（年間総量 ≈ 18-19）。乱数を一切引かない決定論で、`(seed, miracle_log)` が同じなら `full_log` が一致します。神力は得点に**一切数えません**。

**五つの奇跡**:

| 奇跡 | 神力 | 効果（翌朝） | 制約 |
|---|---|---|---|
| 🌦 天候の奇跡 | 1 | 翌日の天気を指定 | その季節の自然な天候のみ・**連日不可**。裏で `next_weather` を呼び**捨て**、自然天候の影（`last_natural_weather`）を `current` に与えるので、RNGストリームは無介入走行と一致 |
| 🎁 恵みのマナ | 2 | 消耗品を翌朝の在庫へ | **ホワイトリスト**（木の実/魚/きのこ/各種の種、各≤3）。木石鉄や加工品は不可（得点直結のため）。ログは「浜に魚が打ち上げられていた」式。**死亡夜は無効**（蘇生なし） |
| 🛖 行商人を呼ぶ | 2 | 翌朝に商人到来 | 前回来訪（自然＋召喚）から **3日クールダウン**。オファーは選べず、**専用RNGチャンネル** `GameRng(seed ^ 0x4D495241)` で抽選するので世界RNGを汚さない |
| 🗲 神託（勅命） | 3 | 翌日の観測**最上位**に `divine_command` を注入（**1回の脳呼び出しで消費**） | **1日1回**。体の声（飢渇）の**次**・作戦の**上**に置かれ、その一手に限り全てに優先。脳を経由するので、混乱した日には**聞き損なう**——記録の正直さとドラマを両立 |
| 💭 夢のお告げ | 1 | 今夜の日記/`memory.add_note` に体験として刻まれる | recent経由で約7日残響 |

**共同カテゴリ（正典の保護）**: 奇跡を**1回でも**使えば、その生は**共同（co-op）**カテゴリになります。無介助の正典記録とは分離され、リザルトに **【共同】奇跡: N回（内訳）** と表示。冒険の書のエントリと `history_table` に `miracles_used` が記録され、家訓の編纂は**介助された生の教訓を割り引いて読み**ます（偽教訓汚染ガード——奇跡で買った長生きの条文が、自力の家訓を汚さないように）。

**操作**:

- ボクセル版: 画面下の **[奇跡]** ボタン（キー **K**）か、承認停止（ピットウォール）の **[奇跡 ◆N]** からパレットを開きます。神力ゲージ（◆◇）・五つのレバー・対象ピッカー（天気は季節内候補、マナはホワイトリスト品目をワンクリック、神託と夢は作戦オーバーレイと同じ入力欄=IME＋CTRL+V）が並び、すべてのボタンが「**明日—**」表記で夜ルールを明示します。HUDにも `神力 ◆◆◆` を常時表示。
- ターミナル版（`--heaven`）: 日次プロンプトで奇跡のサブコマンドが使えます——`!天候 雨` / `!マナ fish` / `!商人` / `!神託 <文>` / `!夢 <文>`。一つの朝に複数キューできます。
...
```text
spl/
  core/      世界、仙人、行動裁定、作物、クラフト、イベント、神のレバー(divine.py)
