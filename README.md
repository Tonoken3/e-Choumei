...
python3 -m spl pixel --difficulty やさしい   # マウスだけで遊べます(LLM不要)
```

- LLMで仙人を観戦したい人は「[LM Studioで遊ぶ](#lm-studioで遊ぶ)」へ。
- 記録に挑む人は `--difficulty ふつう` が正典（カノニカルな基準島）です。
...
島は既定で**容赦なく**調律されています——`ふつう`では、どの隠者も**おおむね19日のうちに尽きます**。これは「同じ脳でどこまで延命できるか」を測るための**カノニカルな基準島**だからです。初めて触る人や、まずマウスだけで雰囲気を味わいたい人のために、夜ごとの消耗をやわらげた `やさしい` と、さらに尖らせた `修羅` を用意しました。

`--difficulty {やさしい,ふつう,修羅}` で選べます（`play` / `simulate` / `pixel` / `evolve`。既定は `ふつう`）。

| きびしさ | 空腹/夜 | 水/夜 | 餓死HP | 渇死HP | 冬の追加税(家未改修) | 開始の追加物資 |
|---|---|---|---|---|---|---|
| **やさしい** | -10 | -14 | -7 | -10 | -2HP / -1正気 | 木の実+3・薪+2 |
| **ふつう**（正典） | -15 | -20 | -10 | -15 | -4HP / -2正気 | なし |
| **修羅** | -18 | -24 | -12 | -18 | -6HP / -3正気 | なし |

- **`ふつう` は記録の正典**です。生存日数の記録（18日記録）やリーダーボード、冒険の書の比較はすべて `ふつう` を基準にします。`ふつう` の数値は難易度機能を入れる前の定数と**バイト単位で一致**しており、同じ seed なら以前とまったく同じ一年が再現されます。
- **入植のしおり（システムプロンプト）は嘘をつきません**。仙人に渡る「夜ごとの致死算術」は、実際に遊んでいる島の数値で動的に綴られます——`修羅` の隠者は `修羅` の算術を、`やさしい` の隠者は緩い算術を読みます。
- 決定論は保たれます: きびしさは固定の消耗を倍率で増減させるだけで**乱数を引かない**ので、`(seed, difficulty)` が同じなら必ず同じ展開になります。
- リザルト画面（ターミナル版・ボクセル版とも）では、`ふつう` 以外のときだけ `きびしさ: やさしい` のように表示され、`--book` の冒険の書にも `difficulty` が記録されます。

```bash
python3 -m spl pixel    --difficulty やさしい           # 初めての人向け（LLM不要）
python3 -m spl simulate --difficulty 修羅 --seed 42      # 修羅の島を高速シミュレート
python3 -m spl simulate --difficulty ふつう --seed 42    # 記録に挑む（正典）
```
